...
using SueTheSecondGeneration.Service;
using SueTheSecondGeneration.Utils;
using System;
...
using static SueTheSecondGeneration.TheSecondGenerationDataSetting;

namespace SueTheSecondGeneration
{
    class TheSecondBusiness
    {
		...
		private static void StartSetTroop(TheSecondGenerationDataSetting setting)
        {
			FillTroops(setting);
		}

		private static void StartSetCompanion(TheSecondGenerationDataSetting setting)
        {
			bool flag4 = setting.HeroNumbers > 0;
			if (flag4)
			{
				List<Hero> list = GameDataGenerateUtils.GeranateHeros(setting.HeroNumbers, setting.HeroFromTier, "", setting.HeroSkillLevel, setting.HeroTakeAllPerks);
				bool flag5 = setting.HeroCoupleMaxNumbers > 0 && setting.HeroNumbers > 1;
				if (flag5)
				{
					GameDataGenerateUtils.SetHeroCoupleAndChildren(list, setting.HeroCoupleMaxNumbers, setting.HeroCoupleChildrenProbability);
				}
			}
		}

		private static void StartSetPlayerFamily(TheSecondGenerationDataSetting setting)
        {
			if (setting.PlayerSkill > 0)
			{
				foreach (SkillObject current in DefaultSkills.GetAllSkills())
				{
		...
		private static void FillTroops(TheSecondGenerationDataSetting setting)
        {
			...
			if (null != soldierArrayList && soldierArrayList.Count > 0)
			{
				foreach (object obj in soldierArrayList)
				{
					...
					{
						TSGValueNamePair pair = (obj as SoldierTemplateData).Soldier;
